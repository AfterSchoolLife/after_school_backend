...
class UsermailerMailer < ApplicationMailer
  ...
  def payment_successful_email(user)
    @user = user
    mail(to: @user.email, subject: 'Payment Confirmation for AfterSchoolLife')
  end

  def payment_failure_email(user)
    @user = user
    mail(to: @user.email, subject: 'Payment Failure for AfterSchoolLife')
  end

  def program_registration_email(user, program)
    @user = user
    @program = program
    mail(to: @user.email, subject: 'Confirmation of Registration for Program')
  end
end
